"""Read the current model and context size from the tail of a session transcript.

Claude Code writes each session to a JSONL file (`transcript_path` in every hook
event). Hook events carry neither the model nor token counts, but every
assistant reply in the transcript does. Only the tail of the file is read, so
the cost stays flat as transcripts grow to many megabytes.
"""
...
def read_last_usage(path: str) -> tuple[str, int] | None:
    ...
    (subagent) lines and lines without usage are skipped. Reads a growing tail
    window (64 KiB, x4 up to 4 MiB) so the whole file is never loaded. Any
    OSError or malformed content yields None rather than an exception.
    """
    try:
...
def fmt_tokens(n: int) -> str:
    """0 -> "", 512 -> "512", 9849 -> "9.8k", 160203 -> "160k"."""
    if n <= 0:
        return ""
    if n < 1000:
        return str(n)
